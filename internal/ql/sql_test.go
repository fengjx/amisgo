...
func TestSelect(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			selector: ql.New("user").Select().As("t").
				Columns("id", "username", "age", "sex", "ctime").
		...
		{
			name: "select alias",
			selector: ql.New("user").Select().As("u").
				Columns("id", "username", "age", "sex"),
			wantSQL: "SELECT u.`id`, u.`username`, u.`age`, u.`sex` FROM `user` AS `u`;",
		},
		{
			name: "select join",
			selector: ql.New("blog").Select().As("u").
...
func TestInsert(t *testing.T) {
	testCases := []struct {
		...
		{
			name:        "insert",
			inserter:    ql.New("user").Insert().Columns("username", "age", "sex"),
		...
		},
		{
			...
			inserter:    ql.New("user").Insert().Columns("username", "age", "sex", "version").OnDuplicateKeyUpdateString("`version` = `version` + 1"),
		...
		},
		{
			...
			inserter:    ql.New("user").Insert().Columns("username", "age", "sex", "version").OnDuplicateKeyUpdate(ql.F("version").Incr(1), ql.F("sex").Val("male")),
		...
		},
		{
			...
			inserter: ql.New("user").Insert().Fields(
...
func TestUpdate(t *testing.T) {
	testCases := []struct {
		...
		{
			name:        "update",
			updater:     ql.New("user").Update().Columns("username", "age"),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantArgs != nil {
				...
				execSQL, args, err = tc.updater.SQLArgs()
				EqualArgs(t, tc.wantArgs, args)
			}
			if tc.wantSQL != "" {
				execSQL, err = tc.updater.SQL()
				assert.Equal(t, tc.wantErr, err)
				if err != nil {
					return
				}
				assert.Equal(t, tc.wantSQL, execSQL)
			}

			if tc.wantNameSQL != "" {
				execSQL, err = tc.updater.NameSQL()
				assert.Equal(t, tc.wantErr, err)
				if err != nil {
					return
				}
				assert.Equal(t, tc.wantNameSQL, execSQL)
			}
		})
	}
}
...
func TestDelete(t *testing.T) {
	testCases := []struct {
		...
		{
			name:    "delete",
			deleter: ql.New("user").Delete(),
			wantSQL: "DELETE FROM `user`;",
			wantErr: ql.ErrDeleteMissWhere,
		},
		{
			name: "delete by id",
			deleter: ql.New("user").Delete().Where(
				ql.SC().And("`id` = ?"),
			),
